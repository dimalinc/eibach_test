            ...
            while (itemNumber<2240) {
                ...
                if (((double) csvRowObjectArrayList.size() % 100 == 0)) {
                    ...
                    for (CsvRowObject csvRowObjectWrite : csvRowObjectArrayList) {
                        ...
                        csv_Writer.writeDataForCustomSeparatorCSV("output_4_" + brand + "_" + dbObject.getItem().getITEM_ID()  + ".csv", stringArrayList);
                    }
                }
            }
            // writeDataForCustomSeparatorCSV("output_"+brand+"_"+arrayListSize+".csv",stringArrayList);
            csv_Writer.writeDataForCustomSeparatorCSV("output_4_full" + brand + "_" + ".csv", stringArrayList);
        } catch (Exception exc) {
            exc.printStackTrace();
        } finally {
            factory.close();
